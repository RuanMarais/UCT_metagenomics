...
def filename_list_generate(file_identifier,
                           folder_path):
    """
    This function finds all files in a folder that contain a specific identifier

    :param file_identifier: The string identifier
    :param folder_path: The folder to search for files
    :return: The list of filenames that contain the identifier
    """
    filenames_output = []
...
def paired_read_list_generate(id_length,
                              file_length,
                              file_id_r1,
                              file_id_r2,
                              filename_list,
                              ...
                              logger):
    """
    This function generates a dictionary of paired reads from a list of filenames

    :param id_length: The string length of the unique identifier which identifies the sample regardless of whether forward or
    reverse read
    :param file_length: The string length of the filename excluding file extension
    :param file_id_r1: The string which identifies forward reads
    :param file_id_r2: The string which identifies reverse reads
    :param filename_list: The list of all filenames
    :param file_folder: The folder where the files are located
    :return: A dictionary with filename of R1 as the key and the item is a tuple of the forward and reverse read
    """
    ...
    for key, files in sample_sorted_dict.items():
        logger.info(f'Retrieving paired reads for sample: {key}')
        # Check if there are two files (R1 and R2) for each sample
        read_1 = None
        read_2 = None
        for file in files:
        ...
        if read_1 is not None and read_2 is not None:
            ...
            if os.path.isfile(read_out_1) and os.path.isfile(read_out_2):
                output = (read_out_1, read_out_2)
                paired_sorted_dict[read_1[:file_length]] = output
                logger.info(f'Paired reads retrieved for sample: {key}')
            else:
                logger.error(f'Paired reads not found for sample: {key}')
        else:
            logger.error(f'Paired reads retrieval failed for sample: {key}')

    return paired_sorted_dict


def populate_krakenfile_dictionary(kraken2_results_directory,
                                   logger):
    """
